...
use reedline::{
    CwdAwareHinter, DefaultPrompt, FileBackedHistory, History, HistoryItem, Reedline, Signal,
};
use std::io;
...
fn create_filled_example_history(home_dir: &str, orig_dir: &str) -> Box<dyn reedline::History> {
    ...
    let mut history: Box<FileBackedHistory> =
        Box::new(FileBackedHistory::new(100).expect("Could not initialise FileBackedHistory"));
