use crate::{
    cprtln, cvprtln, debug_log, lazy_static_var, BuildError, BuildResult, ColorSupport, Lvl,
    TermTheme, Verbosity, V,
};
...
impl Config {
    ...
    fn validate(&self) -> Result<(), BuildError> {
        ...
        self.dependencies
            .validate()
            .map_err(|e| BuildError::Validation(format!("Dependencies validation failed: {e}")))?;
...
pub fn load(context: &Arc<dyn Context>) -> BuildResult<Option<Config>> {
    profile!("load");
...
pub fn edit(context: &dyn Context) -> BuildResult<Option<String>> {
    profile!("edit");
    let config_path = context.get_config_path();
...
pub fn validate_config_format(content: &str) -> Result<(), BuildError> {
    profile!("validate_config_format");
    // Try to parse as generic TOML first
    let doc = content
        .parse::<DocumentMut>()
        .map_err(|e| BuildError::Validation(format!("Invalid TOML syntax: {e}")))?;
