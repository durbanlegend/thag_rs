...
#![expect(unused)]
/// The final version of `thag_rs`'s `colors` module to style messages according to their type.
    ...
    match maybe_color_support {
        ...
        Some(support) => {
            ...
            for variant in Lvl::iter() {
                ...
                // let style = Style::from(&variant);
                let style = term_attrs.style_for_level(variant);
                cvprtln!(
                    variant,
                    V::N,
            ...
            cvprtln!(Lvl::WARN, V::N, "Coloured Warning message\n");
        }
    }
}
