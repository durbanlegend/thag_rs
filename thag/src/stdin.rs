...
use crate::{
    debug_log, vlog, BuildError, CrosstermEventReader, EventReader, KeyDisplayLine, BuildResult, V,
};
use clap::Parser;
use edit::edit_file;
...
pub fn edit<R: EventReader + Debug>(event_reader: &R) -> BuildResult<Vec<String>> {
    ...
    match key_action {
        ...
        KeyAction::Submit => maybe_text.ok_or(BuildError::Cancelled),
        _ => Err(BuildError::FromStr(
...
pub fn edit_history() -> BuildResult<Option<String>> {
    let cargo_home = std::env::var("CARGO_HOME").unwrap_or_else(|_| ".".into());
    let history_path = PathBuf::from(cargo_home).join("rs_stdin_history.json");
