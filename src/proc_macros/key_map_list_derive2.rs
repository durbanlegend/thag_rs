...
#[deluxe(attributes(adjustments))]
pub(crate) struct KeyMappings {
    pub(crate) base: Vec<(i32, String, String)>,
    pub(crate) delete: Vec<String>,
    pub(crate) add: Vec<(i32, String, String)>,
}

pub(crate) fn key_map_list_derive_impl2(
    ...
    let KeyMappings { base, delete, add } = deluxe::extract_attributes(&mut input)?;
    dbg!(base);
    ...
    // Extract the 'base' attribute from the struct
    let mappings_attr = input.attrs.iter().find(|attr| attr.path().is_ident("base"));
    dbg!(mappings_attr);
    ...
    //     None
    // }
    // .expect("Must provide a 'base' attribute");
    // dbg!(mappings_ident);
