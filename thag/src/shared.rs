...
impl BuildState {
    ...
    fn extract_script_info(script_state: &ScriptState) -> BuildResult<(String, String)> {
        profile!("extract_script_info");
        let script = script_state
            .get_script()
            .ok_or(BuildError::NoneOption("No script specified"))?;

        let path = Path::new(&script);
        let filename = path
            .file_name()
            .ok_or(BuildError::NoneOption("No filename specified"))?;

        let source_name = filename
            .to_str()
            .ok_or(BuildError::NoneOption(
...
pub trait EventReader {
    ...
    fn read_event(&self) -> BuildResult<Event>;
    /// Poll for a terminal event.
    ///
    /// # Errors
    ///
    /// This function will bubble up any i/o, `ratatui` or `crossterm` errors encountered.
    fn poll(&self, timeout: Duration) -> BuildResult<bool>;
}
...
impl EventReader for CrosstermEventReader {
    fn read_event(&self) -> BuildResult<Event> {
        ...
        crossterm::event::read().map_err(Into::<BuildError>::into)
    }

    fn poll(&self, timeout: Duration) -> BuildResult<bool> {
        profile_method!("poll");
        crossterm::event::poll(timeout).map_err(Into::<BuildError>::into)
    }
}
