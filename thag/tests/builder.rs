...
use thag::builder::{build, generate, run};
use thag::cmd_args::Cli;
use thag::config::DependencyInference;
use thag::{code_utils, escape_path_for_windows, execute, TMPDIR};
use thag::{BuildState, ProcFlags};
...
fn create_sample_build_state(source_name: &str) -> BuildState {
    ...
    let target_dir_path = TMPDIR.join("thag").join(source_stem);
...
fn test_builder_generate_source_file() {
    ...
    let cargo_toml = format!(
        ...
        [[bin]]
        path = "{}/thag/fib_fac_lite_t/fib_fac_lite_t.rs"
        name = "fib_fac_lite_t"
"#,
...
[[bin]]
path = "{}/thag/bitflags_t/bitflags_t.rs"
name = "bitflags_t"
"#,
        escape_path_for_windows(TMPDIR.display().to_string().as_str())
    );
...
fn test_builder_run_script() {
    ...
    let source_stem: &str = source_name
        .strip_suffix(thag::RS_SUFFIX)
        .expect("Problem stripping Rust suffix");
    let target_dir_path = TMPDIR.join("thag").join(source_stem).join("target/debug");
