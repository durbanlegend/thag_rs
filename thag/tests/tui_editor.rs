...
use thag::tui_editor::{normalize_newlines, History};
use thag::{debug_log, BuildResult, TMPDIR};
...
#[test]
fn test_tui_editor_history_navigate() -> BuildResult<()> {
    set_up();
    let mut history = History::new();
    history.add_entry("first");
    history.add_entry("second");
    ...
    let dir_path = &TMPDIR.join("thag_tests");
