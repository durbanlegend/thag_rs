...
use crate::{
    cprtln, cvprtln, get_verbosity, key, lazy_static_var, regex, vlog, BuildError, BuildState, Cli,
    CrosstermEventReader, EventReader, KeyCombination, KeyDisplayLine, Lvl, ProcFlags, BuildResult,
    V,
};
...
fn tui(
    ...
    let _ = match key_action {
        ...
        KeyAction::Submit => {
            return maybe_text.map_or(Err(BuildError::Cancelled), |v| {
...
pub fn delete(build_state: &BuildState) -> BuildResult<Option<String>> {
    profile!("delete");
...
#[allow(clippy::unnecessary_wraps)]
pub fn edit(source_path: &PathBuf) -> BuildResult<Option<String>> {
...
/// Will return `Err` if there is an error editing the file.
#[allow(clippy::unnecessary_wraps)]
pub fn toml(build_state: &BuildState) -> BuildResult<Option<String>> {
...
#[allow(clippy::unnecessary_wraps)]
pub fn list(build_state: &BuildState) -> BuildResult<Option<String>> {
    profile!("list");
    let source_path = &build_state.source_path;
