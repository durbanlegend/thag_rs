...
fn fib_fn(n: usize) -> impl Iterator<Item = (UBig, UBig)> {
    successors(Some((ubig!(0), ubig!(1))), |(a, b)| Some((b.clone(), a + b)))
   .take(n + 1)
}

let fib_closure = |n: usize| successors(Some((ubig!(0), ubig!(1))), |(a, b)| Some((b.clone(), a + b)))
    .take(n + 1);
...
for (i, (x, y)) in fib_fn(n).zip(fib_closure(n)).enumerate() {
    assert_eq!(x, y);
    println!("F{i} = {}", x.0);
}
