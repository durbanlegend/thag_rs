...
/// Debug version of palette color query with detailed logging
pub fn query_palette_color_debug(index: u8, timeout: Duration) -> Result<Rgb, PaletteError> {
    println!(
        "🐛 DEBUG: Querying color {} with timeout {:?}",
        index, timeout
    );
    ...
    let handle = thread::spawn(move || {
        let result = (|| -> Result<Rgb, PaletteError> {
            ...
            println!("🐛 DEBUG: Raw mode enabled for color {}", index);
            ...
            // println!("🐛 DEBUG: Sending query: {:?}", query.escape_debug());
            ...
            // println!("🐛 DEBUG: Starting read loop for color {}", index);
            while start.elapsed() < timeout {
                match stdin.read(&mut temp_buffer) {
                    Ok(1..) => {
                        ...
                        if buffer.len() % 10 == 0 {
                            let partial = String::from_utf8_lossy(&buffer);
                            // println!(
                            //     "🐛 DEBUG: Buffer length {} for color {}: {:?}",
                            //     buffer.len(),
                            //     index,
                            //     partial.escape_debug()
                            // );
                        }

                        // Only try to parse if we have enough data for a complete response
                        if buffer.len() >= 27 {
                            let response = String::from_utf8_lossy(&buffer);

                            // Check if we have a complete response (contains terminator)
                            if response.contains('\x07') || response.contains("\x1b\\") {
                                // println!(
                                //     "🐛 DEBUG: Complete response for color {}: {:?}",
                                //     index,
                                //     response.escape_debug()
                                // );
                                if let Some(rgb) = try_parse_osc4_response(&response, index) {
                                    // // println!(
                                    //     "🐛 DEBUG: Successfully parsed color {}: RGB({}, {}, {})",
                                    //     index, rgb.r, rgb.g, rgb.b
                                    // );
                                    return Ok(rgb);
                                } else {
                                    // println!(
                                    //     "🐛 DEBUG: Failed to parse response for color {}",
                                    //     index
                                    // );
                                }
                            }
                        ...
                        if buffer.len() > 512 {
                            // println!(
                            //     "🐛 DEBUG: Buffer overflow protection triggered for color {}",
                            //     index
                            // );
                            break;
                        }
                    ...
                    Err(e) => {
                        // println!("🐛 DEBUG: Read error for color {}: {}", index, e);
                        return Err(PaletteError::Io(e));
                    }
                }
            }

            if !buffer.is_empty() {
                let final_response = String::from_utf8_lossy(&buffer);
                // println!(
                //     "🐛 DEBUG: Timeout reached for color {}, final buffer: {:?}",
                //     index,
                //     final_response.escape_debug()
                // );
            } else {
                // println!(
                //     "🐛 DEBUG: Timeout reached for color {}, buffer is empty",
                //     index
                // );
            }
        ...
        eprintln!("🐛 DEBUG: Raw mode disabled for color {}", index);
    ...
    match rx.recv_timeout(timeout + Duration::from_millis(50)) {
        ...
        Err(_) => {
            // println!("🐛 DEBUG: Thread timeout for color {}", index);
            ...
            while start.elapsed() < timeout {
                match stdin.read(&mut temp_buffer) {
                    Ok(1..) => {
                        ...
                        // Only try to parse if we have enough data for a complete response
                        // OSC 4 response format: ESC]4;N;rgb:rrrr/gggg/bbbb BEL
                        // Minimum length: \x1b]4;0;rgb:0000/0000/0000\x07 = 27 characters
                        if buffer.len() >= 27 {
                            let response = String::from_utf8_lossy(&buffer);
...
fn try_parse_osc4_response(response: &str, expected_index: u8) -> Option<Rgb> {
    ...
    if let Some(start_pos) = response.find(&pattern) {
        ...
        // Handle rgb:RRRR/GGGG/BBBB format - wait for complete sequence
        if let Some(rgb_pos) = response_part.find("rgb:") {
            let rgb_data = &response_part[rgb_pos..];

            // Find the end of the RGB sequence (before terminator)
            ...
            if end_pos >= 18 {
                // rgb:rrrr/gggg/bbbb is 18 chars total, rrrr/gggg/bbbb is 15 chars
                let rgb_sequence = &rgb_data[4..end_pos]; // Extract just the rrrr/gggg/bbbb part
                let parts: Vec<&str> = rgb_sequence.split('/').collect();

                // Must have exactly 3 parts, each exactly 4 hex digits
                if parts.len() == 3
                    && parts[0].len() == 4
                    && parts[1].len() == 4
                    && parts[2].len() == 4
...
fn main() -> Result<(), Box<dyn std::error::Error>> {
    ...
    // Test with debug version for first color
    eprintln!("🐛 Testing debug version on color 0:");
    match query_palette_color_debug(0, Duration::from_millis(500)) {
        Ok(rgb) => println!(
            "✅ Debug query successful: RGB({}, {}, {})",
            rgb.r, rgb.g, rgb.b
        ),
        Err(e) => println!("❌ Debug query failed: {:?}", e),
    }
    eprintln!();
...
mod tests {
    ...
    fn test_parse_osc4_response() {
        ...
        // Test incomplete RGB format should return None
        let response3 = "\x1b]4;1;rgb:ff00/80";
        let result3 = try_parse_osc4_response(response3, 1);
        assert_eq!(result3, None);

        // Test malformed RGB format should return None
        let response4 = "\x1b]4;1;rgb:ff0/8000/4000\x07"; // Wrong length
        let result4 = try_parse_osc4_response(response4, 1);
        assert_eq!(result4, None);

        // Test non-hex characters should return None
        let response5 = "\x1b]4;1;rgb:ffgg/8000/4000\x07";
        let result5 = try_parse_osc4_response(response5, 1);
        assert_eq!(result5, None);

        // Test wrong index should return None
        let response6 = "\x1b]4;2;rgb:ff00/8000/4000\x07";
        let result6 = try_parse_osc4_response(response6, 1); // Looking for index 1
        assert_eq!(result6, None);

        // Test partial hex format should return None
        let response7 = "\x1b]4;0;#ff80";
        let result7 = try_parse_osc4_response(response7, 0);
        assert_eq!(result7, None);

        // Test real terminal response format
        let response8 = "\x1b]4;0;rgb:1818/1818/1818\x07";
        let result8 = try_parse_osc4_response(response8, 0);
        assert_eq!(result8, Some(Rgb::new(24, 24, 24)));
    }
