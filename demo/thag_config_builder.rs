...
fn add_doc_comments(toml_str: &str, doc_comments: &[(String, String)]) -> String {
    println!("Adding doc comments to TOML. Available comments:");
    for (path, comment) in doc_comments {
        println!("  Path: '{}', Comment: '{}'", path, comment);
    }

    let mut result = String::from("# Generated by thag_config_builder\n\n");
    let mut current_section = String::new();
    let mut in_feature_overrides = false;
    ...
    for line in toml_str.lines() {
        ...
        if trimmed.starts_with('[') && trimmed.ends_with(']') {
            // Parse section name, handling nested sections
            let section = trimmed[1..trimmed.len() - 1].to_string();
            current_section = if section.contains('.') {
                in_feature_overrides = true;
                section.split('.').next().unwrap().to_string()
            } else {
                in_feature_overrides = false;
                section
            };

            println!(
                "Found section: '{}' (nested: {})",
                current_section, in_feature_overrides
            );

            // Add section doc comment
            if let Some((_, comment)) = doc_comments.iter().find(|(path, _)| {
                println!(
                    "Comparing section '{}' with path '{}'",
        ...
        if !in_feature_overrides && !trimmed.is_empty() && trimmed.contains('=') {
            if let Some(field_name) = trimmed.split('=').next() {
                let field_name = field_name.trim();
                let full_path = format!("{}.{}", current_section, field_name);
                println!("Found field: '{}', full path: '{}'", field_name, full_path);

                // Add field doc comment
                if let Some((_, comment)) = doc_comments.iter().find(|(path, _)| {
                    println!("Comparing field '{}' with path '{}'", full_path, path);
                    *path == full_path
                }) {
                    for comment_line in comment.lines() {
                        result.push_str(&format!("# {}\n", comment_line));
