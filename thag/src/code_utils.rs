...
use crate::{
    ...
    vlog, Ast, BuildError, BuildState, Cli, Lvl, BuildResult, BUILT_IN_CRATES, DYNAMIC_SUBDIR,
...
pub fn read_file_contents(path: &Path) -> BuildResult<String> {
...
pub fn path_to_str(path: &Path) -> BuildResult<String> {
    profile!("path_to_str");
    let string = path
        .to_path_buf()
        .into_os_string()
        .into_string()
        .map_err(BuildError::OsString)?;
...
pub fn display_output(output: &Output) -> BuildResult<()> {
...
) -> BuildResult<Option<(&PathBuf, SystemTime)>> {
...
pub fn write_source(to_rs_path: &PathBuf, rs_source: &str) -> BuildResult<fs::File> {
...
pub fn create_temp_source_file() -> BuildResult<PathBuf> {
...
pub fn is_main_fn_returning_unit(file: &File) -> BuildResult<bool> {
