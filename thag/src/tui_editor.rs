...
use crate::{
    debug_log, key, regex, BuildError, BuildResult, EventReader, KeyCombination, KeyDisplayLine,
    Lvl,
};
use crossterm::event::{
...
/// # Errors
///
pub fn resolve_term<'a>() -> BuildResult<Option<ManagedTerminal<'a>>> {
...
impl History {
    ...
    pub fn save_to_file(&mut self, path: &PathBuf) -> BuildResult<()> {
...
) -> BuildResult<(KeyAction, Option<Vec<String>>)>
where
    R: EventReader + Debug,
    F: Fn(
        KeyEvent,
        ...
        let event = if test_env.is_ok() {
            ...
            maybe_term.as_mut().map_or_else(
                ...
                |term| {
                    ...
                    event.map_err(Into::<BuildError>::into)
                },
            )?
        };
...
pub fn maybe_enable_raw_mode() -> BuildResult<()> {
...
pub fn reset_term(
    mut term: Terminal<CrosstermBackend<std::io::StdoutLock<'_>>>,
) -> BuildResult<()> {
    profile!("reset_term");
    disable_raw_mode()?;
    crossterm::execute!(
